...
template<class Y>
inline void sp_pointer_construct(Y *p, woodycxx::smart_prt::detail::shared_count &pn) {
...
template<class T>
inline typename shared_ptr<T>::element_type *get_pointer(shared_ptr<T> const &p) {
  return p.get();
}
}
}
